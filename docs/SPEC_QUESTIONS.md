...
- **Spec section:** Sparse file logical size (spec section involving `SPARSE_FILES` and `Uncompressed Size`)
  - **Issue:** The spec states that `Uncompressed Size` represents the "fully reconstructed logical file after all applicable transformations have been reversed during decoding." However, for sparse files the payload contains only the non-hole data segments; the logical file size (including holes) may differ from the sum of data segment lengths. The spec does not define a separate field for the full sparse logical size, and whether `Uncompressed Size` refers to the sum of data segments or the full logical size is ambiguous.
  - **Current conservative implementation:** The implementation treats `Uncompressed Size` as the size of the decoded data payload (sum of sparse extent lengths). The sparse logical size is derived during `read_all_logical_files` as `max(extent.offset + extent.length)` across all extents. A trailing sparse hole beyond the last extent would cause the derived logical size to be smaller than the actual logical size.
  - **Interoperability risk:** If another implementation stores `Uncompressed Size` as the logical file size (including holes), archives produced by that implementation will fail the current size validation check in `next_entry`. Conversely, archives produced by this implementation will be misinterpreted by the other.
  - **Follow-up needed:** The spec must explicitly state whether `Uncompressed Size` for sparse entries equals (a) the sum of all sparse extent data lengths, or (b) the full logical file size including holes. If it is (b), a separate "sparse data payload size" field must also be defined. Additionally, the spec should clarify whether a trailing sparse hole (past the last extent) is representable and how the boundary is encoded.
