...
# Conformance Profile Status (M1-M12c.1)

This document describes **reference implementation coverage** and **known gaps** after M12c.1 documentation/public-claims hardening.

This is an **implemented profile** report, not a claim of full standard conformance.
It is not a certification or interoperability guarantee.

Compatibility expectations and non-guarantees are documented in `docs/COMPATIBILITY.md`.

## Current conformance status and limits (M12c.1)

* Where vectors are implemented and non-deferred, the current target is SAR Protocol v1.0 behavior as defined by `specification.md`.
* Conformance checks are useful implementation evidence, but not a complete conformance oracle.
* Deferred vectors and deferred functionality remain explicitly out of scope for current conformance claims.
* Partition/multi-volume conformance execution remains deferred with `sar-partition`.
* No full conformance certification claim is made.
...
- This pass is M12a-stream-cp - **M12b fuzzing was not started by that pass**.
...
M12b fuzzing/hardening coverage is documented separately in:
- `fuzz/README.md`
- `fuzz/CORPUS.md`
- `fuzz/RUNS.md`

Fuzzing remains non-exhaustive and does not replace conformance-vector coverage.
The M12a manifest format and stable status identifiers remain suitable for
continued fuzzing-related extension without schema churn.
...
- M12b: fuzzing and malicious corpus baseline/extended hardening completed; continued campaigns may proceed in parallel.
- M12c: documentation closeout (M12c.1 completed; M12c.2/M12c.3 continue separately).
