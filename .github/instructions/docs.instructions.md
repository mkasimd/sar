...
* `docs/API.md` is generated from `docs/machine-readable/MACHINE_READABLE_API.json`; do not edit it by hand.

When public APIs change:

* update `docs/machine-readable/MACHINE_READABLE_API.json`
* keep it valid against `docs/machine-readable/MACHINE_READABLE_API.schema.json`
...
Do not add arbitrary fields to `docs/machine-readable/MACHINE_READABLE_API.json`. Follow the schema.

Do not make false Standard Compliance claims.

When APIs move or are removed, update `docs/machine-readable/MACHINE_READABLE_API.json` to describe the current public API surface. Do not preserve stale entries in the API inventory unless they remain public compatibility shims.
