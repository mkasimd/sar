...
fn parses_valid_minimal_global_header() {
    ...
    let (parsed, consumed) = parse_global_header(&bytes).expect("parse header");
...
fn rejects_wrong_magic() {
    ...
    bytes[0] = b'X';
    let err = parse_global_header(&bytes).expect_err("must fail");
...
fn supports_structural_kms_but_not_custom_mode() {
    ...
    let bytes = write_global_header(&header).expect("write header");
    let err = parse_global_header(&bytes).expect_err("custom mode should be unsupported");
